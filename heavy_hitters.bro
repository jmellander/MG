#
# $Id: heavy_hitters.bro,v 1.1 2018/10/30 17:48:30 melland Exp melland $
#
# Heavy Hitters
...
global HeavyHitters::recurring_connection_observe: event(id: conn_id);
...
type interval_rec: record
	...
	filter_function: function(ip: addr): bool
		&default = function(ip: addr): bool {return T;};
	...
	report_function: function(start: time, end: time, val: string): string
		&default = function(start: time, end: time, val: string): string
			...
			if (val == "Grand Total")
				retval = fmt("Heavy Hitters report from %s to %s",
	...
	grouping_function: function(ip: addr): string
		&default = function(ip: addr): string {return fmt("%s",ip);};
...
event HeavyHitters::FirstTime() &priority = 10
	{
...
function observe(c: connection)
	...
	# Add to Sumstats
		...
		{
		#print current_time(), network_time(),"OBSERVE",orig, new_orig_bytes;
		SumStats::observe( "HeavyHitters", [$host=orig], [$num=new_orig_bytes] );
		}
        if (new_resp_bytes >= MIN_SAMPLE)
		{
		#print current_time(), network_time(),"OBSERVE",resp, new_resp_bytes;
	        SumStats::observe( "HeavyHitters", [$host=resp], [$num=new_resp_bytes] );
		}
	}
...
function observation_schedule(id: conn_id)
	...
	if (connection_exists(id))
		...
		schedule next_observation - now
			{ HeavyHitters::recurring_connection_observe(id) };
		}
	}

# Recurring connection observation event
event HeavyHitters::recurring_connection_observe(id: conn_id)
	{
...
function do_report(which: count)
	...
	#local output = sort_ips(intervals[which]$hosts, intervals[which]$sort_buckets, 9999);
	local rec: HeavyHitters::Info;
	...
	for (i in output)
		...
		Log::write(HeavyHitters::LOG, rec);
	...
	if (rec$bytes_out > 0)	Log::write(HeavyHitters::LOG, rec);
...
event HeavyHitters::FirstTime()
	...
	local red: SumStats::Reducer = [$stream="HeavyHitters", $apply=set(SumStats::SUM)];

	hh = [$name="HeavyHitters", 
		...
		$epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) =
			...
			local r = result["HeavyHitters"];
			#print current_time(),network_time(),"HeavyHitters, r=",r;
			local ip = key$host;
			local ip_str: string;
			local amt:count = double_to_count(r$sum);
			local which: count;
			...
			for (which in intervals)
				...
				if (intervals[which]$filter_function(ip))
					...
					# Here, do Grand Totals for the interval
...
event bro_init() &priority=5
	...
	#  is available
	schedule 0usec { HeavyHitters::FirstTime() };

	# Create the logging stream. This adds a default filter automatically.
	Log::create_stream(HeavyHitters::LOG, [$columns=Info, $path="heavyhitters"]);
	}
